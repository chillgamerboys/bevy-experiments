...
fn source_fixture(root: &Path) -> Result<String> {
    ...
    for (name, bytes) in archive_files()? {
        ...
        let path = root.join("gameskills").join(name);
        fs::create_dir_all(path.parent().ok_or("parent")?)?;
        fs::write(path, bytes)?;
    }
    let bundle = root.join("gameskills/cli/bundle");
...
fn pinned_source_export_checks_full_commit_tags_cleanliness_and_prepared_bytes() -> Result {
    ...
    fs::write(
        source.join("gameskills/plugins/gameskills/local-output.log"),
    ...
    assert!(!root
        ...
        .exists());
    let skill = source.join("gameskills/plugins/gameskills/skills/plan/SKILL.md");
    fs::OpenOptions::new()
        .append(true)
        .open(&skill)?
        .write_all(b"\nchanged\n")?;
...
#[test]
fn historical_source_layout_exports_the_same_portable_bundle() -> Result {
    let root = tempfile::tempdir()?;
    let source = root.path().join("source");
    source_fixture(&source)?;
    fs::rename(source.join("gameskills/plugins"), source.join("plugins"))?;
    fs::create_dir(source.join("tools"))?;
    fs::rename(
        source.join("gameskills/cli"),
        source.join("tools/gameskills-cli"),
    )?;
    git(&source, &["add", "."])?;
    git(&source, &["commit", "-qm", "Historical source layout"])?;
    ...
    assert!(root
        ...
        .join("export/plugins/gameskills/skills/plan/SKILL.md")
        .is_file());
    assert!(!root.path().join("export/gameskills").exists());
    Ok(())
}
