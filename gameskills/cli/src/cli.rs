...
const USAGE_HELP: &str = "gameskills usage mark TASK --log SESSION.jsonl --stage NAME --role coordinator|worker [--skill SKILL]...\ngameskills usage finish TASK --log SESSION.jsonl\ngameskills usage checkpoint TASK --log SESSION.jsonl --phase start|end --role coordinator|worker [--attempt ID]\ngameskills usage import --file RECEIPT.json\ngameskills usage report TASK [--rates RATES.json] [--details]\nReport observed task/thread deltas; missing telemetry/prices remain unavailable.";
...
enum Operation {
    ...
    /// Preserve the task endpoint and observe delivery against current source.
    #[command(after_help = DELIVERY_HELP)]
    Delivery {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<OsString>,
    },
            ...
            let result = match family {
                ...
                "plan" | "queue" | "run" | "evidence" | "delivery" => {
                    crate::installation::ready_config(&root).and_then(|config| {
                        if family == "delivery" {
                            crate::delivery::execute(&root, &config, &arguments)
                        } else if matches!(family, "plan" | "queue") {
