...
pub(super) fn export(root: &Path, args: &[String]) -> Result<Value, String> {
    ...
    let bundle = if let Some(source) = source {
        ...
        // Source checkouts have changed layout; exported/installed bundle paths have not.
        let modern = !git(
            &source,
            &[
                "ls-tree",
        ...
        .is_empty();
        let (plugin_root, bundle_root) = if modern {
            ("gameskills/plugins", "gameskills/cli/bundle")
        } else {
            ("plugins", "tools/gameskills-cli/bundle")
        };
        for command in [
            vec![
                "diff",
            ...
            .filter(|entry| !entry.is_empty())
        {
            ...
            let archive_path = path.strip_prefix("gameskills/").unwrap_or(path);
            ...
            }) || archive_path == "plugins/gameskills/catalog.json";
            if !instruction {
                continue;
            }
            instruction_path(archive_path)?;
