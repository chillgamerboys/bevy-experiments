...
| Native UI | `bevy-gamekit-ui`, `GameUiPlugin`, `GameUiSystems` | View/intent ownership, activation order, focus and semantic metrics |
...
| Test mechanics | `bevy-gamekit-testing`, `TestAppBuilder`, `HeadlessUiPlugin` | Exact plugins/features, frame progression and bounds |
| Discrete sequencing | `bevy-gamekit-turns`, `TurnOrder` | Roster/cursor/round transitions, removal and serialization |
| Session/networking | `bevy-gamekit-session`, `bevy-gamekit-multiplayer` | Identity, credential persistence, authorization and reconnect |
| Discovery | `bevy-gamekit-discovery`, `DiscoveryJoinRoute` | Sanitized observations, provider lifecycle and transport handoff |
| Hex algorithms | `bevy-gamekit-hex` | Coordinate/layout math without board rules |
