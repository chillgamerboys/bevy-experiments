...
`GameUiPlugin` owns semantic metrics, native focus and entity activation, not a skin.
Attach a game-local action component to `UiAction`; translate `UiActivated.entity` in a
consumer system ordered after `GameUiSystems::EmitActivations`, using an independent
message reader/cursor rather than draining the shared stream. Opt into semantic
painting with `GameUiSkinPlugin` and per-entity `UiSkin`; use `UiSkinOverrides` for
individual surfaces. Supply branding through `UiTheme`, `UiFonts` and `UiTextStyle`.
Declare control baselines with `UiControlMetrics` and managed spacing with `UiSpacing`;
do not multiply resolved dimensions on each resize. `UiMotionPreference` is shared,
but games decide which animations it affects. Keep views and typed intents local.
...
Import identities directly from it, not a transport compatibility re-export.
`SessionAdmissionAuthority` offers a credential before authorizing: the client
persists it atomically, acknowledges it, and only then may the game admit commands.
Games bind attempts to connections and own seats, capacity and stale-message handling.
