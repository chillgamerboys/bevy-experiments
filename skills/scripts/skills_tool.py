...
def generated_hashes(rendered: dict[Path, bytes]) -> dict[str, str]:
    """Use platform-independent relative names for the exact generated bytes."""

    return {
        path.as_posix(): digest(data)
        for path, data in sorted(rendered.items(), key=lambda item: item[0].as_posix())
    }
...
def verified_base(target: Path, manifest: dict[str, object]) -> dict[Path, bytes]:
    ...
    for name, checksum in generated.items():
        if not isinstance(name, str) or "\\" in name:
            raise ValueError("generated manifest paths must use canonical '/' separators; regenerate adoption metadata from the pinned source")
        if PurePosixPath(name).as_posix() != name:
            raise ValueError(f"non-canonical generated path: {name}")
        safe_generated_path(target, Path(name))
        if not isinstance(checksum, str) or not re.fullmatch(r"[0-9a-f]{64}", checksum):
            raise ValueError("invalid generated file hash")
    base = read_tree(target / BASE)
    actual = generated_hashes(base)
    if actual != generated:
...
def write_manifest(target: Path, repository: str, revision: str, resolved_sha: str, rendered: dict[Path, bytes]) -> None:
    ...
    payload = {
        ...
        "skills": list(SKILLS),
        "generated": generated_hashes(rendered),
    }
