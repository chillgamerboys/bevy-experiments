...
`bundle prepare --revision <full-commit-id>` generates `gameskills/cli/bundle/`
from committed canonical instructions plus this package's `bundle-compatibility.json`.
Commit changed inputs first. `gameskills/plugins/` remains the human-edited skill source.
...
`devtools/migration-contracts.json` and compares complete accounting with the frozen
...
The [CI guide](docs/ci.md) describes selection and verification. This internal
package is not part of an adopter installation; the [GameSkills CLI](../gameskills/cli/README.md)
provides adopter-facing workflows.
