    ...
    for (client, relative) in [
        ...
        for entry in entries {
            ...
            if !resolves_to(
                ...
                &root.join("gameskills/plugins").join(name),
            ) {
                failures.push(format!(
...
pub fn validate(root: &Path) -> Vec<String> {
    ...
    let mut failures = Vec::new();
    let plugin_root = root.join("gameskills/plugins");
    ...
    let path = root.join("gameskills/plugins/gameskills/catalog.json");
    ...
    failures.extend(validate_scenarios(
        &root.join("devtools/tests/fixtures/catalog/gameskills-scenarios.json"),
    ));
    failures
}
