...
const INPUT: &str = "devtools/bundle-compatibility.json";
/// Generated Cargo-local payload directory; never an installation.
pub const OUTPUT: &str = "gameskills/cli/bundle";
...
fn selected(path: &str) -> Result<bool, String> {
    ...
    for (package, _) in EXPECTED_SKILLS {
        if let Some(relative) = path.strip_prefix(&format!("gameskills/plugins/{package}/")) {
            if matches!(
                relative,
...
fn inputs(root: &Path, revision: &str) -> Result<BTreeMap<String, Vec<u8>>, String> {
    ...
    for (package, skills) in EXPECTED_SKILLS {
        ...
        {
            if !files.contains_key(&format!("gameskills/plugins/{package}/{required}")) {
                return Err(format!("missing canonical input: {package}/{required}"));
            }
        }
    }
    if !files.contains_key(INPUT)
        || !files.contains_key("gameskills/plugins/gameskills/catalog.json")
    {
...
fn generate(root: &Path, revision: &str, require_commit: bool) -> Result<Prepared, String> {
    ...
    // Repository ownership does not change the portable installation layout.
    let mut files: BTreeMap<String, Vec<u8>> = files
        .into_iter()
        .map(|(path, bytes)| {
            let portable = path
                .strip_prefix("gameskills/")
                .ok_or("instruction source is outside gameskills")?
                .to_owned();
            Ok((portable, bytes))
        })
...
pub fn verify_package(root: &Path, archive: Option<&Path>) -> Result<Value, String> {
    ...
    let manifest: toml::Value = toml::from_str(&crate::support::read_text(
        &root.join("gameskills/cli/Cargo.toml"),
    )?)
    .map_err(|error| error.to_string())?;
