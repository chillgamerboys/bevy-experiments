...
fn test_reverse_normal_dev_target_build_consumers() -> TestResult {
    let fixture = Fixture::new()?;
    let value = fixture.changed(&["gamekit/ui/src/lib.rs"])?;
    assert_packages(
        &value,
        &[
            "bevy-gamekit",
...
fn test_missing_history_and_bad_graph_select_full() -> TestResult {
    ...
    fixture.write("gamekit/ui/Cargo.toml", "broken TOML")?;
...
fn rust_ci_implementation_and_regressions_are_shared_inputs() -> TestResult {
    ...
    for path in [
        ...
        "devtools/src/ci/mod.rs",
        "devtools/src/ci/checks.rs",
        "devtools/tests/ci_routing.rs",
        "devtools/tests/fixtures/ci/selection.json",
    ] {
        assert_full(&fixture.changed(&[path])?);
        fixture.reset()?;
    }
...
fn removed_include_and_deleted_compiled_doc_retain_old_consumers() -> TestResult {
    ...
    fixture.rebase()?;
    fixture.write("gamekit/ui/src/lib.rs", "// include removed\n")?;
...
fn member_component_globs_and_exclusions_preserve_ownership() -> TestResult {
    ...
    fixture.write("gamekit/unused/Cargo.toml", "excluded invalid TOML")?;
    fixture.rebase()?;
    let value = fixture.changed(&["gamekit/ui/src/lib.rs"])?;
    assert_packages(
        &value,
...
#[test]
fn relocated_product_docs_and_plugins_keep_their_scoped_routes() -> TestResult {
    let fixture = Fixture::new()?;
    let docs = fixture.changed(&[
        "gamekit/README.md",
        "gamekit/docs/ui.md",
        "gameskills/README.md",
        "gameskills/docs/development.md",
    ])?;
    assert!(
        !docs.full && !flags(&docs).into_iter().any(|flag| flag),
        "{docs:?}"
    );
    fixture.reset()?;
...
#[test]
fn source_layout_rename_preserves_base_and_head_paths_and_runs_full_checks() -> TestResult {
    let mut fixture = Fixture::new()?;
    fixture.git(&["mv", "gamekit", "crates"])?;
    fixture.replace("Cargo.toml", "gamekit/", "crates/")?;
    for game in ["labyrinth", "deckbuilder", "carterfight"] {
        fixture.replace(&format!("games/{game}/Cargo.toml"), "gamekit/", "crates/")?;
    }
    fixture.rebase()?;
    fixture.git(&["mv", "crates", "gamekit"])?;
    fixture.replace("Cargo.toml", "crates/", "gamekit/")?;
    for game in ["labyrinth", "deckbuilder", "carterfight"] {
        fixture.replace(&format!("games/{game}/Cargo.toml"), "crates/", "gamekit/")?;
    }
    let selection = fixture.select()?;
    assert!(
        selection.full && selection.rust && selection.skills && selection.distribution,
        "{selection:?}"
    );
    Ok(())
}
