...
fn fixture() -> Result<(tempfile::TempDir, String), Box<dyn Error>> {
    ...
    write(
        root,
        "devtools/bundle-compatibility.json",
    ...
    for (package, skills) in EXPECTED_SKILLS {
        for client in ["codex", "claude"] {
            write(
                root,
                &format!("gameskills/plugins/{package}/.{client}-plugin/plugin.json"),
                format!("{{\"name\":{package:?}}}\n"),
            )?;
        }
        ...
        write(
            root,
            &format!("gameskills/plugins/{package}/references/context.md"),
            "# Context\n",
        )?;
    }
    write(
        root,
        "gameskills/plugins/gameskills/catalog.json",
        "{\"version\":\"0.1.0-dev.1\"}\n",
    )?;
    write(
        root,
        "gameskills/plugins/gameskills/scripts/gameskills.py",
        "print('not bundled')\n",
    )?;
    write(
        root,
        "gameskills/plugins/gameskills/runtime/old.py",
        "# not bundled\n",
    )?;
...
fn repeated_preparation_and_export_share_exact_payload_and_manifest() -> TestResult {
    ...
    for (package, _) in EXPECTED_SKILLS {
        for client in ["claude", "codex"] {
            ...
            assert_eq!(
                files.get(&path),
                Some(&std::fs::read(root.join("gameskills").join(&path))?)
            );
        }
    }
...
fn unrelated_commits_keep_pin_valid_but_changed_inputs_require_new_pin() -> TestResult {
    ...
    write(
        root,
        "gameskills/plugins/gameskills/references/context.md",
        "Changed\n",
    )?;
...
fn missing_instruction_unsupported_asset_and_bad_compatibility_fail() -> TestResult {
    for (path, bytes) in [
        (
            "gameskills/plugins/gameskills/skills/plan/code.py",
            "# forbidden asset",
        ),
        (
            "devtools/bundle-compatibility.json",
    ...
    std::fs::remove_file(
        temp.path()
            .join("gameskills/plugins/gameskills/skills/plan/SKILL.md"),
    )?;
    let revision = commit(temp.path())?;
    assert!(bundle::prepare(temp.path(), &revision).is_err());
    Ok(())
...
fn symlink_sources_and_outputs_are_never_followed() -> TestResult {
    ...
    std::fs::create_dir_all(root.join("gameskills/cli"))?;
    ...
    let source = root.join("gameskills/plugins/gameskills/references/context.md");
...
fn checkout_line_endings_do_not_change_canonical_blob_identity() -> TestResult {
    ...
    let path = root.join("gameskills/plugins/gameskills/references/context.md");
...
fn real_cli_cargo_archive_must_contain_the_verified_payload() -> TestResult {
    ...
    let root = temp.path();
    bundle::prepare(root, &revision)?;
    write(root, "gameskills/cli/Cargo.toml", "[package]\nname = \"gameskills-cli\"\nversion = \"0.1.0\"\nedition = \"2021\"\npublish = false\n[workspace]\n")?;
    write(
        root,
        "gameskills/cli/src/lib.rs",
        "// Inert packaging probe\n",
    )?;
    let cli = root.join("gameskills/cli");
