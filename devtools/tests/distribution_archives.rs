...
archive_probe_helper = { path = "gamekit/helper" }
"#,
    )?;
    write(
        root,
        "gamekit/helper/Cargo.toml",
        r#"[package]
...
"#,
    ...
    write(
        root,
        "gamekit/helper/src/lib.rs",
        "pub fn answer() -> u32 { 42 }\n",
    )?;
    write(
        root,
        "gamekit/facade/Cargo.toml",
        r#"[package]
name = "bevy-gamekit"
version.workspace = true
edition.workspace = true
...
"#,
    ...
    write(
        root,
        "gamekit/facade/src/lib.rs",
        "#[cfg(feature = \"hex\")]\npub fn answer() -> u32 { archive_probe_helper::answer() }\n",
    )?;
    write(root, "gamekit/facade/excluded.rs", "not selected\n")?;
...
fn real_two_crate_archives_compile_after_staging_is_removed() -> Result<(), Box<dyn Error>> {
    ...
    let original_facade = std::fs::read(root.join("gamekit/facade/Cargo.toml"))?;
    let mut staged = None;
    ...
    assert_eq!(
        std::fs::read(root.join("gamekit/facade/Cargo.toml"))?,
        original_facade
    );
