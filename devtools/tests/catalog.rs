...
const CATALOG: &str = "gameskills/plugins/gameskills/catalog.json";
const SCENARIOS: &str = "devtools/tests/fixtures/catalog/gameskills-scenarios.json";
...
impl Candidate {
    fn new() -> Result<Self, Box<dyn Error>> {
        ...
        for &(package, skills) in EXPECTED_SKILLS {
            ...
            for skill in skills {
                fixture.write(
                    &format!("gameskills/plugins/{package}/skills/{skill}/SKILL.md"),
                    &format!("---\nname: {skill}\ndescription: {DESCRIPTION}\n---\n\n{BODY}"),
                )?;
            }
            fixture.write(
                &format!("gameskills/plugins/{package}/references/local.md"),
                "# Local reference\n\nA maintained package-local reference.\n",
            )?;
            fixture.write_json(&format!("gameskills/plugins/{package}/.claude-plugin/plugin.json"), &json!({"name": package, "version": version, "description": description, "author": {"name": "Fixture"}}))?;
            fixture.write_json(&format!("gameskills/plugins/{package}/.codex-plugin/plugin.json"), &json!({"name": package, "version": version, "description": description, "author": {"name": "Fixture"}, "skills": "./skills/", "interface": {"displayName": "Fixture", "capabilities": []}}))?;
            codex.push(json!({"name": package, "source": {"source": "local", "path": format!("./gameskills/plugins/{package}")}}));
            claude.push(json!({"name": package, "source": format!("./gameskills/plugins/{package}"), "version": version}));
        }
        fixture.write_json(
            CATALOG,
    ...
    fn skill(&self) -> PathBuf {
        self.path("gameskills/plugins/gameskills/skills/plan/SKILL.md")
    }
...
fn extra_native_skill_and_missing_native_body_are_detected() -> TestResult {
    let fixture = Candidate::new()?;
    fixture.write(
        "gameskills/plugins/gameskills/skills/inject/SKILL.md",
        &std::fs::read_to_string(fixture.skill())?.replace("name: plan", "name: inject"),
    )?;
    std::fs::remove_file(fixture.path("gameskills/plugins/gameskills/skills/debug/SKILL.md"))?;
...
fn optional_package_cannot_discover_sibling_core_skills() -> TestResult {
    let fixture = Candidate::new()?;
    fixture.edit(
        "gameskills/plugins/gameskills-ui/.codex-plugin/plugin.json",
        "/skills",
...
fn manifest_identity_and_versions_match_catalog() -> TestResult {
    let fixture = Candidate::new()?;
    let manifest = "gameskills/plugins/gameskills-ui/.claude-plugin/plugin.json";
...
fn native_metadata_types_are_checked_without_assuming_client_execution() -> TestResult {
    let fixture = Candidate::new()?;
    fixture.edit(
        "gameskills/plugins/gameskills/.codex-plugin/plugin.json",
        "/interface",
...
fn marketplace_inventory_and_source_mapping_are_both_checked() -> TestResult {
    ...
    fixture.edit(
        ".claude-plugin/marketplace.json",
        "/plugins/1/source",
        json!("./gameskills/plugins/gameskills"),
    )?;
...
fn existing_encoded_link_outside_package_is_rejected() -> TestResult {
    let fixture = Candidate::new()?;
    fixture.append(
        &fixture.path("gameskills/plugins/gameskills-ui/skills/build-ui/SKILL.md"),
...
fn valid_space_and_fragment_local_links_are_portable() -> TestResult {
    let fixture = Candidate::new()?;
    fixture.write(
        "gameskills/plugins/gameskills/references/space name.md",
        "# Heading\n",
    )?;
...
fn symlink_cannot_make_an_external_reference_look_package_local() -> TestResult {
    ...
    std::os::unix::fs::symlink(
        ...
        fixture.path("gameskills/plugins/gameskills-ui/references/escape.md"),
    )?;
    fixture.append(
        &fixture.path("gameskills/plugins/gameskills-ui/skills/verify-ui/SKILL.md"),
...
fn symlink_loop_is_a_validation_error_without_a_traceback() -> TestResult {
    ...
    std::os::unix::fs::symlink(
        "loop.md",
        fixture.path("gameskills/plugins/gameskills/references/loop.md"),
    )?;
...
fn optional_ui_metadata_cannot_point_at_another_skill() -> TestResult {
    let fixture = Candidate::new()?;
    fixture.write("gameskills/plugins/gameskills/skills/plan/agents/openai.yaml", "interface:\n  display_name: \"Plan\"\n  short_description: \"Plan a scoped game task\"\n  default_prompt: \"Use $release for this task.\"\n")?;
...
fn unsupported_local_paths_and_nested_discovery_are_rejected() -> TestResult {
    ...
    let fixture = Candidate::new()?;
    fixture.write(
        "gameskills/plugins/gameskills/skills/plan/nested/SKILL.md",
        BODY,
    )?;
    fixture.assert_failure("discovered SKILL.md inventory mismatch");
    fixture.write("gameskills/plugins/unexpected/readme.md", BODY)?;
...
fn directory_symlinks_and_contained_leaf_symlinks_are_explicitly_rejected() -> TestResult {
    ...
    std::os::unix::fs::symlink(
        "local.md",
        fixture.path("gameskills/plugins/gameskills/references/alias.md"),
    )?;
    fixture.assert_failure("native package entries must not be symlinks");
    std::os::unix::fs::symlink(
        fixture.path("gameskills/plugins/gameskills/references"),
        fixture.path("gameskills/plugins/gameskills-ui/references/outside-directory"),
    )?;
    fixture.assert_failure("symlink escapes native package");
    let package = fixture.path("gameskills/plugins/gameskills-ui");
    ...
    let fixture = Candidate::new()?;
    std::fs::rename(
        fixture.path("gameskills/plugins"),
        fixture.path("outside-plugins"),
    )?;
    std::os::unix::fs::symlink(
        fixture.path("outside-plugins"),
        fixture.path("gameskills/plugins"),
