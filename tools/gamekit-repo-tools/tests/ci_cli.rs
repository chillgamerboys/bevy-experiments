...
impl Repo {
    fn new() -> Self {
        ...
        std::fs::create_dir_all(root.join("crates/core/src")).expect("source directory");
        for (path, source) in [
            (
                "Cargo.toml",
                "[workspace]\nmembers = ['crates/*']\n[profile.ci]\ninherits = 'dev'\n",
            ...
            ("crates/core/src/lib.rs", "pub fn example() {}\n"),
...
fn actual_cargo_child_failure_streams_logs_and_stops_the_job() {
    let repo = Repo::new();
    std::fs::write(repo.directory.path().join("crates/core/src/lib.rs"),
