...
cap = { path = "crates/bevy_game_hex", default-features = false }
...
fn fixture() -> Result<tempfile::TempDir, Box<dyn Error>> {
    ...
    for (directory, package) in [
        ("crates/bevy_gamekit", "bevy-gamekit"),
        ("crates/bevy_game_hex", "bevy_game_hex"),
        ("games/labyrinth", "labyrinth"),
        ("games/labyrinth/rules", "labyrinth-rules"),
        ("tools/gamekit-repo-tools", "gamekit-repo-tools"),
    ] {
        write(
            root,
...
fn staging_removes_game_membership_preserving_shared_settings() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        ...
        Some(&toml::Value::Array(vec!["crates/*".into()]))
    );
...
fn alternate_workspace_toml_preserves_unrelated_values() -> Result<(), Box<dyn Error>> {
    for source in [
        ...
        "workspace.resolver = '3'\nworkspace.members = ['tools/*']\nworkspace.default-members = ['tools/cli']\nworkspace.dependencies.cap = { path = 'crates/cap' }\n",
        "workspace = {resolver='3', members=['games/*'], default-members=['games/a'], exclude=['crates/a'], package={version='1.0.0'}, lints={rust={unsafe_code='forbid'}}}\n",
    ] {
        ...
        workspace.insert("members".into(), toml::Value::Array(vec!["crates/*".into()]));
...
fn staging_rejects_linked_crates_and_hidden_nonlibrary_packages() -> Result<(), Box<dyn Error>> {
    use std::os::unix::fs::symlink;
    for relative in ["crates/linked", "tools/linked", "games/linked"] {
        let directory = fixture()?;
        symlink(
    ...
    let directory = fixture()?;
    let source = directory.path().join("crates/bevy_game_hex/src/lib.rs");
...
fn all_cases_stage_only_cargo_sources_embed_fixture_and_seed_lock() -> Result<(), Box<dyn Error>> {
    ...
    let report = check_with_runner(&root, Case::All, |cwd, args| {
        ...
        match args.first().and_then(|arg| arg.to_str()) {
            ...
            Some("tree") => {
                ...
                assert!(!library.join("tools").exists());
                assert!(!root.join("scripts").exists());
                for capability in ["bevy_gamekit", "bevy_game_hex"] {
                    let capability = library.join("crates").join(capability);
                ...
                assert_eq!(
                    ...
                    library.join("crates/bevy_gamekit").to_str()
                );
                assert_eq!(
                    facade
    ...
    assert_eq!(report.packages, ["bevy_game_hex", "bevy-gamekit"]);
...
fn missing_and_malformed_inputs_fail_before_cargo() -> Result<(), Box<dyn Error>> {
    for (path, replacement) in [
        ...
        (
            "tools/gamekit-repo-tools/Cargo.toml",
            Some("[package]\nname = false"),
        ),
        (
            "crates/bevy_game_hex/Cargo.toml",
            Some("[package]\nname = 'hex'\n[dependencies]\nbad={"),
        ),
        (
            "crates/bevy_game_hex/Cargo.toml",
