...
fn ci_input(path: &str) -> bool {
    matches!(
        ...
        || [
            ...
            "tools/gamekit-repo-tools/src/ci/",
            "tools/gamekit-repo-tools/tests/ci",
            "tools/gamekit-repo-tools/tests/fixtures/ci/",
        ]
        .iter()
...
fn compare(root: &Path, base: &str, head: &str) -> Result<Selection, String> {
    ...
    for path in &result.paths {
        ...
        let prose = matches!(
            path.as_str(),
            "README.md" | "LICENSE" | "LICENSE-MIT" | "LICENSE-APACHE" | "CHANGELOG.md"
        );
            ...
            .any(|prefix| path.starts_with(prefix))
        {
            ...
            } else if owner.is_some() || path.starts_with("docs/") || prose {
                result
                    .reasons
            ...
            affected.extend(
                new.packages
                    .iter()
                    .filter(|(_, path)| path.starts_with("crates/"))
