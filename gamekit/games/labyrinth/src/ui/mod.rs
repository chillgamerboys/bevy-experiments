...
use bevy::prelude::*;
use bevy_game_ui::{
    GameUiSystems, ResolvedUiMetrics, UiActivated, UiFonts, UiScaleMode, UiScalePreference,
    UiTextChanged, UiTextField, UiTextRole, UiTheme,
};
...
struct UiState {
    ...
    lan: bool,
    tailnet: bool,
    selected_session: Option<bevy_game_multiplayer::SessionId>,
...
enum Action {
    ...
    Browse,
    JoinDiscovered,
    Session(bevy_game_multiplayer::SessionId),
    Reconnect,
    Leave,
    ToggleLan,
    ToggleTailnet,
...
fn collect_actions(world: &mut World) {
    let actions = {
        let mut messages = world.resource_mut::<Messages<UiActivated>>();
        messages
            .drain()
...
fn label(
    ...
    world
        .spawn((Name::new(name.to_owned()), bundle, ChildOf(parent)))
        .id()
}
