...
pub(crate) fn emit_activations(world: &mut World) {
    ...
    if world.resource::<ButtonInput<KeyCode>>().any_just_pressed([
        KeyCode::Enter,
        KeyCode::NumpadEnter,
        KeyCode::Space,
    ]) {
...
pub(crate) fn emit_text_field_messages(world: &mut World) {
    let changes = {
        let mut query = world
            .query_filtered::<(Entity, &EditableText), (With<UiTextField>, Changed<EditableText>)>(
            );
        query
            .iter(world)
            .map(|(entity, editable)| UiTextChanged {
                entity,
    ...
    if !world
        ...
        .just_pressed(Key::Enter)
    {
        return;
    }
    let Some(entity) = world.resource::<InputFocus>().get() else {
        return;
    };
    if world.get::<UiTextField>(entity).is_none() || !activation_eligible(world, entity) {
        return;
    }
    let Some(editable) = world.get::<EditableText>(entity) else {
        return;
    };
    if !editable.is_composing() {
        let value = editable_value(editable);
        world.write_message(UiTextSubmitted { entity, value });
    }
}
...
pub(crate) fn prepare_actions(
    ...
    for (entity, name, tab_index, existing, labeled) in &controls {
        if existing.is_none() {
            let logical = tab_index.map_or(0, |index| index.0);
            commands.entity(entity).insert(UiTabOrder(logical));
        }
        if !labeled {
            if let Some(name) = name {
                commands
                    .entity(entity)
                    .insert(AccessibleLabel::new(name.as_str().to_owned()));
            }
        }
    }
}
...
pub(crate) fn sync_action_reachability(world: &mut World) {
    let actions = {
        let mut query = world
            .query_filtered::<(Entity, &UiTabOrder), Or<(With<UiAction>, With<UiTextField>)>>();
        query
            .iter(world)
            .map(|(entity, index)| (entity, index.0))
            .collect::<Vec<_>>()
    };
    for (entity, logical_index) in actions {
        ...
        if let Some(mut current) = world.get_mut::<TabIndex>(entity) {
            ...
            // A declared order is enough: custom controls need not know about
            // Bevy's resolved component, including when it was removed later.
...
fn first_reachable_action(world: &World, root: Entity) -> Option<Entity> {
    ...
    while let Some(entity) = stack.pop() {
        let order = world.get::<UiTabOrder>(entity).map_or_else(
            || world.get::<TabIndex>(entity).map_or(0, |index| index.0),
            |index| index.0,
        );
        if order >= 0
            && (world.get::<UiAction>(entity).is_some()
                || world.get::<UiTextField>(entity).is_some())
            && is_reachable(world, entity)
        {
            candidates.push((order, entity));
        }
        if let Some(children) = world.get::<Children>(entity) {
            stack.extend(children.iter().rev());
        }
    }
    // Stable sort preserves document order among equal keyboard priorities.
