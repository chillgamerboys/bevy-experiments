...
//!
//! # Appearance is opt-in
//!
//! [`GameUiPlugin`] supplies mechanics and measurements, not a game skin. A
//! transparent scene overlay or white pixel-art dialogue can use the same
//! controls as a dark card browser. Add [`GameUiSkinPlugin`] once in a game's
//! composition root, then attach [`UiSkin`] only to surfaces it should paint.
//!
//! ```
//! use bevy::prelude::*;
...
//!         button("Confirm"),
//!         UiSkin::Control,
//!         UiControlMetrics { min_size: Vec2::new(160.0, 48.0) },
//!         UiSpacing {
//!             padding: Some(UiInsets::axes(UiSpace::Units(1.5), UiSpace::Units(1.0))),
//!             ..default()
//!         },
//!     ));
//!     // This control keeps its game-owned colors even when the skin is installed.
//!     commands.spawn(button("Custom"))
//!         .insert(BackgroundColor(Color::WHITE));
//! }
//! ```
//!
//! Measurements always resolve from declarations, not previously scaled output.
//! [`UiTextRole`] opts into typography but does not own `TextColor`; [`UiSpacing`]
...
use metrics::{apply_control_metrics, apply_spacing, apply_typography, prepare_control_metrics};
pub use metrics::{
    UiControlMetrics, UiInsets, UiMotionPreference, UiSpace, UiSpacing, UiTextStyle,
};
pub use style::{GameUiSkinPlugin, UiSkin, UiSkinOverrides};
...
/// Installs responsive metrics, activation, and keyboard focus, without painting.
///
/// Semantic typography and explicitly declared spacing scale independently of
/// colors. Add [`GameUiSkinPlugin`] and [`UiSkin`] only for surfaces that should
/// use the optional neutral skin.
...
pub enum GameUiSystems {
    ...
    EmitActivations,
    /// Apply focus, reachability, and semantic measurements before layout.
    Present,
    /// Apply opt-in appearance after measurements and before layout.
    Style,
}
...
impl Plugin for GameUiPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(TabNavigationPlugin)
            ...
            .configure_sets(
                PostUpdate,
                (GameUiSystems::Present, GameUiSystems::Style)
                    .chain()
            ...
            .add_systems(
                Update,
                (emit_activations, emit_text_field_messages)
                    .chain()
...
/// Declared keyboard order, independent of temporary reachability.
///
/// Initially captured from `TabIndex` if absent. Change this component for a
/// dynamic order; the resolved `TabIndex` is managed by the focus system.
/// Negative values exclude a control from sequential keyboard navigation.
#[derive(Component, Debug, Clone, Copy, PartialEq, Eq)]
pub struct UiTabOrder(pub i32);

/// Creates a transparent, keyboard-navigable full-screen root.
/// Add [`UiSkin::Screen`] to opt into the neutral screen background.
#[must_use]
...
/// Creates an unpainted column panel with convenience geometry.
#[must_use]
...
/// Creates an unpainted card surface without imposing card dimensions.
#[must_use]
...
/// Creates an unpainted action button with a 44-pixel minimum target.
/// Games own its dimensions, palette, and label content.
#[must_use]
...
/// Creates a transparent blocking full-screen modal scope.
/// Add [`UiSkin::Modal`] for a dimmed backdrop, or supply a game-owned background.
#[must_use]
